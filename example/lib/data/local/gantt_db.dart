  ...
  static Future<SqliteCrdt> _init() async {
    ...
    final db = await SqliteCrdt.open(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        if (oldVersion < 14) {
          try {
            await db.execute('ALTER TABLE tasks ADD COLUMN is_auto_scheduled INTEGER DEFAULT 1');
          } catch (_) {}
        }

        if (oldVersion < 15) {
          try {
            await db.execute('ALTER TABLE tasks ADD COLUMN propagates_move_to_children INTEGER DEFAULT 1');
          } catch (_) {}
          try {
        ...
        if (oldVersion < 17) {
          try {
            ...
            print('Migration to v17 (last_updated_by) error (likely already exists): $e');
          }
        }
